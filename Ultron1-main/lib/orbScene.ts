...
export interface OrbSceneApi {
  ...
  zoomOut(): void;
  resetView(): void;
  /** Feed a 0..1 audio/voice level in; the orb glows, pulses and speeds up with it. */
...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  // ═══════════════════════════════════════════════
  // VOICE REACTIVITY — external code (mic input / TTS output) feeds a
  // 0..1 level in via setAudioLevel(); everything below just reads
  // `audioLevel`, so when it's 0 the orb behaves exactly as before.
  // ═══════════════════════════════════════════════
  let audioLevelTarget = 0;
  let audioLevel = 0;

  function setAudioLevel(level: number) {
    audioLevelTarget = THREE.MathUtils.clamp(level, 0, 1);
  }
  ...
  function animate() {
    ...
    // Smooth the incoming voice/audio level toward its target
    audioLevel += (audioLevelTarget - audioLevel) * 0.15;

    // Outer shell rotation — spins up slightly while "listening/speaking"
    outerShell.rotation.y += 0.0015 + audioLevel * 0.004;
    ...
    const coreScale = 1 + surge + Math.sin(t * 5) * 0.05 + audioLevel * 0.5;
    ...
    coreSphereMat.opacity = Math.min(1, coreOpacity + audioLevel * 0.55);
    glowSphere.scale.setScalar(1 + surge * 0.8 + audioLevel * 0.7);
    glowSphereMat.opacity = Math.max(0, (0.03 + surge * 0.08) * (1 - fadeOut * 0.9)) + audioLevel * 0.3;
    // Icosahedron wireframe stays visible even when glow fades
    icoWire.scale.setScalar(1 + surge * 0.6 + audioLevel * 0.35);
    icoWireMat.opacity = Math.min(1, 0.5 + surge * 0.4 + audioLevel * 0.5);
    ...
    // Scan rings sweeping
    const scanY1 = Math.sin(t * (0.4 + audioLevel * 0.5)) * R1;
    scanRing1.position.y = scanY1;
    const scanS1 = Math.sqrt(Math.max(0, R1 * R1 - scanY1 * scanY1)) / R1;
    scanRing1.scale.set(scanS1, scanS1, 1);
    (scanRing1.material as THREE.MeshBasicMaterial).opacity = (0.2 + audioLevel * 0.35) * scanS1;

    const scanY2 = Math.sin(t * (0.6 + audioLevel * 0.6) + 2) * R3;
    scanRing2.position.y = scanY2;
    const scanS2 = Math.sqrt(Math.max(0, R3 * R3 - scanY2 * scanY2)) / R3;
    scanRing2.scale.set(scanS2, scanS2, 1);
    (scanRing2.material as THREE.MeshBasicMaterial).opacity = (0.15 + audioLevel * 0.3) * scanS2;
    ...
    // Bloom pulse — flares up while listening/speaking
    bloom.strength = 1.6 + Math.sin(t * 0.8) * 0.3 + audioLevel * 1.4;
